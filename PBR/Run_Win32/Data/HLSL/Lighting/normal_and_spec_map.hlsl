...
float4 FragmentFunction(vertex_to_fragment_t data) : SV_Target0
{
	...
	for (int index = 0; index < 16; ++index){
		...
		//return float4(directionToLight, 1.0f);
		//return float4(dot3, dot3, dot3, 1.0f);
        ...
        //return float4(pow(specDot3, SPECULAR.power), pow(specDot3, SPECULAR.power), pow(specDot3, SPECULAR.power)	, 1.0f);
    ...
    for (int dirLightIndex = 0; dirLightIndex < 4; ++dirLightIndex)
    {
        ...
        float directionalLightFactor = saturate(dot(-directionNormalized, surfaceNormal));

        directionalLightFactors += directionalLightFactor * directionalLightColor;
    }

    //return saturate(pointLightFactors);

    float4 diffuseFactor = saturate(ambientFactor + pointLightFactors + directionalLightFactors);
