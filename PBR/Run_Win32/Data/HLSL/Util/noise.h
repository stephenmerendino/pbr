...
float Compute4dPerlinNoiseZeroToOne(float x, float y, float z, float t, unsigned int seed)
{
	float noise = Compute4dPerlinNoiseNegOneToOne(x, y, z, t, seed);
	noise *= 0.5f;
	noise += 0.5f;
	return noise;
}
...
{
	float noise = Compute4dPerlinNoiseNegOneToOne(posX, posY, posZ, posT, scale, numOctaves, octavePersistence, octaveScale, renormalize, seed);
	noise *= 0.5f;
	noise += 0.5f;
	return noise;
}
