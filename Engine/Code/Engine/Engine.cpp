...
void engine_init(engine_startup_t params)
{
	...
	Rgba clear_color = DEFAULT_CLEAR_COLOR;
	ConfigGetRgba(&clear_color, params.config_key_clear_color);

	// setup the renderering system
	g_theRenderer->Setup(width, height);
	g_theRenderer->m_clear_color = clear_color;
...
void engine_tick(float ds)
{
	...
	console_update(ds);
    RemoteCommandService::get_instance()->update();
