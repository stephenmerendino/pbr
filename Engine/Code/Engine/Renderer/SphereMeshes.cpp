...
void Meshes::build_uv_sphere(MeshBuilder& mb, const Vector3& center, float radius, const Rgba& color, unsigned int slices)
{
	...
	float slice_uv_step = 1.0f / (float)(slices);
	for(float v = 0.0f; v <= 1.0f; v += slice_uv_step){
		for(float u = 0.0f; u <= 1.0f; u += slice_uv_step){
