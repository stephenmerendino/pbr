...
public:
	...
	void set_material_id(uint mat_id);
...
{
	return write(d.start_index) && write(d.count) && write(d.primitive_type) && write(d.uses_index_buffer) && write(d.mat_id);
}
...
{
	return read(d.start_index) && read(d.count) && read(d.primitive_type) && read(d.uses_index_buffer) && read(d.mat_id);
}