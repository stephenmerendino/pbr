...
void ConsoleDisplay::Render()
{
	...
	if(IsOpen()){
		...
		g_theRenderer->EnableDepth(false, true);
...
void ConsoleDisplay::DrawBackground() const
{
	...
	// Green Background
	g_theRenderer->DrawQuad2d(0.0f, 0.0f, 1.0f, 1.0f, AABB2::ZERO_TO_ONE, CONSOLE_ACCENT_BG_COLOR);

	// Log
	g_theRenderer->DrawQuad2d(0.001f, 0.004f, 0.999f, 0.995f, AABB2::ZERO_TO_ONE, CONSOLE_MAIN_BG_COLOR);
...
void ConsoleDisplay::RenderConsoleInputHighlighting() const
{
	...
	int selectedTextStartIndex = (int)m_inputBuffer.size() + m_startSelectOffset;
	int selectedTextEndIndex = (int)m_inputBuffer.size() + m_endSelectOffset;
