...
bool LoadBinaryFileToBuffer(unsigned char** out_buffer, const char* filePath)
{
	...
	*out_buffer = new unsigned char[filesize + 1];
...
bool SaveBufferToBinaryFile(unsigned const char* buffer, const char* filePath)
{
	// Open output file
	FILE* file;
...
bool LoadBinaryFileToBuffer(unsigned char** out_buffer, size_t* out_buffer_size, const char* filePath)
{
	FILE* file;
	errno_t errorCode = fopen_s(&file, filePath, "rb");
	if(errorCode != 0){
	...
	*out_buffer_size = size;
	...
	size_t bytesRead = fread(*out_buffer, 1, size, file);
...
bool SaveBufferToBinaryFile(const unsigned char* buffer, size_t buffer_size, const char* filePath)
{
	FILE* file;
	errno_t errorCode = fopen_s(&file, filePath, "wb");
	if(errorCode != 0){
	...
	size_t bytesWritten = fwrite(buffer, 1, buffer_size, file);

	std::string errorMessage = Stringf("Error, could not write all data to file %s\n", filePath);
	ASSERT_OR_DIE(bytesWritten == buffer_size, errorMessage.c_str());
