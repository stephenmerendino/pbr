...
//
#if defined( DISABLE_ASSERTS )
#define DIE( errorMessageText ) { (void)( condition ); }
#else
#define DIE( errorMessageText )												\
{																								\
	FatalError( __FILE__,  __FUNCTION__, __LINE__, errorMessageText, "Error" );			\
}
#endif

#define ASSERT_RETURN_VALUE(expr, ret_val) \
{                                          \
    if(expr){}                             \
    else{                                  \
        return ret_val;                    \
