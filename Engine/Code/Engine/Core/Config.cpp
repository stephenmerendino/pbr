...
public:
	...
	ConfigSetting(const char* name, const Vector3& vec3)
		:m_name(nullptr)
		,m_vec3Value(vec3)
		,m_type(CONFIG_TYPE_VEC3)
	{
		SetName(name);
	}
...
private:
	...
	void ParseNextToken(char** out_token)
	{
		...
		bool isColor = (m_rawBuffer[m_curIndex] == 'c' && m_rawBuffer[m_curIndex + 1] == '<');
		bool isVec3 = (m_rawBuffer[m_curIndex] == 'v' && m_rawBuffer[m_curIndex + 1] == '<');
		...
		while(endIndex < m_size){
			...
			if(!isStringLiteral && !isColor && !isVec3 && IsCharTokenSeperator(m_rawBuffer[endIndex])){
				break;
			}
			...
			if((isVec3 || isColor) && m_rawBuffer[endIndex] == '>'){
...
bool LoadConfigFileToRawBuffer(RawConfigBuffer* out_configFileBuffer, const char* filename)
{
	...
	(*out_configFileBuffer).m_size = (unsigned int)(filesize + 1);
...
void LoadCommandLineToRawConfigBuffer(RawConfigBuffer* out_buffer, const char* commandLine)
{
	...
	*out_buffer = RawConfigBuffer();
	out_buffer->m_size = (unsigned int)(stringLen + 1);
...
ConfigType GetConfigTypeForRawValue(const char* rawValue)
{
	...
	if(stringLength > 2){
		...
		if(rawValue[0] == 'c' && rawValue[1] == '<'){
			return CONFIG_TYPE_RGBA;
		}

		if(rawValue[0] == 'v' && rawValue[1] == '<'){
			return CONFIG_TYPE_VEC3;
		}
	}
...
bool IsNumberListSeperator(char c)
{
	return (c == ' ' || c == ',');
}
...
unsigned char GetNextRgbaValue_Bracket(unsigned int* out_curIndex, const char* rawValue)
{
	...
	while(!IsNumberListSeperator(rawValue[*out_curIndex])){
...
float GetNextVec3Value(unsigned int* out_curIndex, const char* rawValue)
{
	// Make sure we haven't hit the end bracket
	if(rawValue[*out_curIndex] == '>'){
		return 0.0f;
	...
	int index = 0;
	char raw_float[25];
	memset(raw_float, 0, 25);

	while(!IsNumberListSeperator(rawValue[*out_curIndex])){
		raw_float[index] = rawValue[*out_curIndex];
	...
	float value = (float)atof(raw_float);
	return value;
}
...
void ConvertRawValueToVec3(Vector3* out_vec3, const char* rawValue)
{
	unsigned int curIndex = 2;

	float x = GetNextVec3Value(&curIndex, rawValue);
	float y = GetNextVec3Value(&curIndex, rawValue);
	float z = GetNextVec3Value(&curIndex, rawValue);

	*out_vec3 = Vector3(x, y, z);
}
...
void ConvertAndSetRawValue(const char* key, const char* rawValue)
{
	...
	switch(valueType){
		...
		case CONFIG_TYPE_VEC3:
		{
			Vector3 v;
			ConvertRawValueToVec3(&v, rawValue);
			ConfigSet(key, v);
		}
	}
}
...
void ConfigSet(const char* name, const void* value, ConfigType type)
{
	...
	if(!setting){
		switch(type){
			...
			case CONFIG_TYPE_VEC3:		s_configSettings.push_back(new ConfigSetting(name, *((Vector3*)value)));	break;
		}
	} else{
		setting->m_type = type;
		switch(type){
			...
			case CONFIG_TYPE_VEC3:		setting->m_vec3Value    =   *((Vector3*)value);		break;
		}
	}
}
...
bool ConfigGetValue(void* out_value, const char* name, ConfigType type)
{
	...
	if(setting && setting->m_type == type){
		switch(type){
			...
			case CONFIG_TYPE_VEC3:		*((Vector3*)out_value)		= setting->m_vec3Value;				break;
		}
...
void ConfigSet(const char* name, const Vector3& vec)
{
	ConfigSet(name, (void*)&vec, CONFIG_TYPE_VEC3);
}
...
bool ConfigGetVec3(Vector3* out_value, const char* name)
{
	return ConfigGetValue(out_value, name, CONFIG_TYPE_VEC3);
}
...
void PrintTypeHeading(FILE* file, ConfigType type)
{
	switch (type)
	{
		case CONFIG_TYPE_STRING:	fprintf(file, "# [STRING]\n");		break;
		case CONFIG_TYPE_BOOL:		fprintf(file, "# [BOOL]\n");		break;
		case CONFIG_TYPE_INT:		fprintf(file, "# [INT]\n");		break;
		case CONFIG_TYPE_FLOAT:		fprintf(file, "# [FLOAT]\n");		break;
		case CONFIG_TYPE_RGBA:		fprintf(file, "# [COLOR]\n");		break;
		case CONFIG_TYPE_VEC3:		fprintf(file, "# [VEC3]\n");		break;
	}
}
...
bool ConfigSystemSaveToFile(const char* filename)
{
	...
	for(; iter != s_configSettings.end(); ++iter){
		...
		switch(setting->m_type)
		{
			...
			case CONFIG_TYPE_RGBA:		fprintf(file, "%s = c<%u, %u, %u, %u>\n", setting->m_name, setting->m_rgbaValue.r,
																										  setting->m_rgbaValue.g,
																										  setting->m_rgbaValue.b,
																										  setting->m_rgbaValue.a);		break;
			case CONFIG_TYPE_VEC3:		fprintf(file, "%s = v<%f, %f, %f>\n", setting->m_name, setting->m_vec3Value.x,
																							  setting->m_vec3Value.y,
																							  setting->m_vec3Value.z); break;
		}
	}
...
COMMAND(list_config, "list all loaded config options")
{
	...
	for(; iter != s_configSettings.end(); ++iter){
		...
		case CONFIG_TYPE_RGBA:		console_info("%s = <%u, %u, %u, %u> (rgba)", setting->m_name, setting->m_rgbaValue.r,
																										  ...
																										  setting->m_rgbaValue.b,
																										  setting->m_rgbaValue.a);	break;
		case CONFIG_TYPE_VEC3:		console_info("%s = <%f, %f, %f> (vec3)", setting->m_name, setting->m_vec3Value.x,
																								  setting->m_vec3Value.y,
																								  setting->m_vec3Value.z); break;
		}
	}
