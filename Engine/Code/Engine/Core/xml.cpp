...
XMLElement xml::create_root_element(const char* root_name)
{
	return XMLNode::createXMLTopNode(root_name);
}

XMLElement xml::add_child(XMLElement& element, const char* child_name)
{
	XMLElement new_node = create_root_element(child_name);
	element.addChild(new_node);
	return new_node;
}

void xml::set_attribute(XMLElement& element, const char* attr_name, const char* attr_value)
{
	element.addAttribute(attr_name, attr_value);
}

void xml::save_root_to_file(XMLElement& element, const char* filename)
{
	element.writeToFile(filename);
}