...
namespace xml
{
	...
	XMLElement create_root_element(const char* root_name);
	XMLElement add_child(XMLElement& element, const char* child_name);
	void set_attribute(XMLElement& element, const char* attr_name, const char* attr_value);
	void save_root_to_file(XMLElement& element, const char* filename);
}