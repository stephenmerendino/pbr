...
Shader::~Shader()
{
	...
	if(m_raster_state != g_theRenderer->DEFAULT_RASTER_STATE){
		SAFE_DELETE(m_raster_state);
	}
	if(m_depth_stencil_state != g_theRenderer->DEFAULT_DEPTH_STENCIL_STATE){
		SAFE_DELETE(m_depth_stencil_state);
	}
	if(m_blend_state != g_theRenderer->DEFAULT_BLEND_STATE){
		SAFE_DELETE(m_blend_state);
	}
}
...
{
	xml::validate_xml_node(shader_program_xml, "vertex, hull, domain, geometry, fragment", "src, geometry_stage", "", "");
	...
	if(xml::element_has_attribute(shader_program_xml, "src")){
        ...
        bool load_geometry_stage = xml::parse_xml_attribute(shader_program_xml, "geometry_stage", false);
		return new ShaderProgram(g_theRenderer->m_device, single_src_filename.c_str(), load_geometry_stage);
	}
	// read in vertex and fragment children elements
        ...
        std::string hull_filename_xml;
        if(xml::element_has_child(shader_program_xml, "hull")){
    		XMLElement hull_xml = xml::get_element_child(shader_program_xml, "hull");
    		xml::validate_xml_node(hull_xml, "", "src", "", "src");
    		hull_filename_xml = xml::parse_xml_attribute(hull_xml, "src", hull_filename_xml);
        }

        std::string domain_filename_xml;
        if(xml::element_has_child(shader_program_xml, "domain")){
    		XMLElement domain_xml = xml::get_element_child(shader_program_xml, "domain");
    		xml::validate_xml_node(domain_xml, "", "src", "", "src");
    		domain_filename_xml = xml::parse_xml_attribute(domain_xml, "src", domain_filename_xml);
        }
		...
		return new ShaderProgram(g_theRenderer->m_device, vertex_filename.c_str(), 
														  hull_filename_xml.c_str(), 
														  domain_filename_xml.c_str(), 
														  geometry_filename_xml.c_str(), 
