...
void SamplerDescription::SetIsComparison(bool is_comparison)
{
	m_is_comparison = is_comparison;
}
...
static D3D11_FILTER DXGetFilterMode(FilterMode minFilter, FilterMode magFilter, FilterMode mipFilter, bool is_comparison = false)
{
	if(!is_comparison){

		if (minFilter == FILTER_MODE_POINT){
			if (magFilter == FILTER_MODE_POINT){
				if(mipFilter == FILTER_MODE_POINT){
...
bool Sampler::CreateSampler(const SamplerDescription& samplerDescription)
{
	...
	desc.Filter = DXGetFilterMode(samplerDescription.m_minFilter, samplerDescription.m_magFilter, samplerDescription.m_mipFilter, samplerDescription.m_is_comparison);
